...
namespace Gemstone.Gemstone
{
    public class ColoredBoards : MonoBehaviour
    {
        ...
        private const int STUMP_INDEX = 3;
        private const int FOREST_INDEX = 6;

        private Material forestMaterial;
        private Material stumpMaterial;
        private GameObject computerMonitor;

        private readonly Dictionary<string, GameObject> activeCustomBoards = new Dictionary<string, GameObject>();
        private readonly List<Renderer> networkTriggerScreens = new List<Renderer>();
        private readonly Dictionary<string, GameObject> sceneHierarchyCache = new Dictionary<string, GameObject>(StringComparer.OrdinalIgnoreCase);
        ...
        private void UpdateBaseMapLeaderboards()
        {
            ...
            {
                GameObject stumpBoard = GetLeaderboardMesh(treeRoom.transform, STUMP_INDEX);
                if (stumpBoard != null)
                {
            ...
            {
                GameObject forestBoard = GetLeaderboardMesh(forestRoom.transform, FOREST_INDEX);
                if (forestBoard != null)
                {
        ...
        private void ProcessNetworkTriggers()
        {
            if (!triggerScanComplete)
            {
                ...
                if (triggers != null)
                {
                    foreach (GorillaNetworkJoinTrigger trigger in triggers)
                    {
                        ...
                        {
                            if (rend != null && rend.gameObject.name.IndexOf("screen", StringComparison.OrdinalIgnoreCase) >= 0)
                            {
            ...
            for (int i = 0; i < networkTriggerScreens.Count; i++)
            {
                Renderer screen = networkTriggerScreens[i];
                if (screen != null && screen.gameObject.activeInHierarchy)
                {
                    if (screen.sharedMaterial != BoardMaterial)
                    {
        ...
        private void UpdateComputerMonitorMaterial()
        {
            ...
            {
                computerMonitor = FetchCachedElement("Environment Objects/LocalObjects_Prefab/TreeRoom/TreeRoomInteractables/GorillaComputerObject/ComputerUI/monitor/monitorScreen");
            }
        ...
        public void GenerateCustomSurface(string sceneId, MapBoardConfig details)
        {
            try
            {
                ...
                planeSurface.transform.localRotation = Quaternion.Euler(details.OffsetRotation ?? new Vector3(270f, 0f, 0f));
        ...
        {
            return sceneHierarchyCache.TryGetValue(computedKey, out GameObject discoveredObject) && discoveredObject != null
                ? discoveredObject
                : null;
        }
        ...
        private static readonly Dictionary<string, MapBoardConfig> MapRegister = new Dictionary<string, MapBoardConfig>
        {
